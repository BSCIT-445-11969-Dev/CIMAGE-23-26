    ...
    const getCart = () => {
        ...
        for (const key in cartItems) {
            ...
            if (product) {
                tempArray.push({
                    ...product,
                 quantity: cartItems[key], // create new object instead of mutating
            });
        }
    }
...
const getUserAddresses = async () => {
    try {
        const { data } = await axios.get('/api/address/get'); // No body needed, middleware handles it
        if (data.success) {
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                <button onClick={()=>{navigate('/products'); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-green-600 font-medium">
                    <img src={assets.arrow_right_icon_colored} alt="arror" className="group-hover:-translate-x-1 transition" />
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <select onChange={e => setPaymentOption(e.target.value)} className="w-full border border-emerald-800 bg-white px-3 py-2 mt-2 outline-none">
                ...
                <button onClick={placeOrder}  disabled={!user} className="w-full py-3 mt-6 cursor-pointer bg-emerald-900 text-white font-medium hover:bg-green-600 transition">
